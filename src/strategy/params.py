...
from src.strategy.markout_tracker import MarkoutTracker
from src.strategy.param_math import _clamp, _ewma, _ramp_unit
from src.strategy.param_observer import observe_symbol_state
from src.strategy.sweep_reversion import SweepReversionEngine
...
class AdaptiveParameterProvider:
    ...
    sweep_engine: SweepReversionEngine = field(default_factory=SweepReversionEngine)
    ...
    def _derive_from_state(
        ...
        *,
        ...
        # then snaps back to it. On sweep detection the engine front-loads a passive
        # standing ladder AT the short MA (queue priority for the snap-back), and
        # fades physics-momentum chasing the sweep direction while it's underway.
        # See sweep_reversion.py for the full mechanism and the long-MA drift bias.
        sweep_outputs = self.sweep_engine.compute(
            state=state,
            adaptive_state=adaptive_state,
            tick_size=tick_size,
            now_ns=now_ns,
        ...
        physics_momentum_shift_ticks = sweep_outputs.physics_momentum_shift_ticks
        return_zscore = sweep_outputs.return_zscore
        mean_reversion_score = sweep_outputs.mean_reversion_score
        noise_fade_shift_ticks = sweep_outputs.noise_fade_shift_ticks
        mr_error_fast_ticks = sweep_outputs.mr_error_fast_ticks
        mr_error_medium_ticks = sweep_outputs.mr_error_medium_ticks
        mr_error_slow_ticks = sweep_outputs.mr_error_slow_ticks
        mr_reversion_quality_fast = sweep_outputs.mr_reversion_quality_fast
        mr_reversion_quality_medium = sweep_outputs.mr_reversion_quality_medium
        mr_reversion_quality_slow = sweep_outputs.mr_reversion_quality_slow
        mr_crossed_fast = sweep_outputs.mr_crossed_fast
        mr_crossed_medium = sweep_outputs.mr_crossed_medium
        mr_crossed_slow = sweep_outputs.mr_crossed_slow
        mr_cross_direction = sweep_outputs.mr_cross_direction
        mr_cross_depth = sweep_outputs.mr_cross_depth
        mr_cross_anchor_price = sweep_outputs.mr_cross_anchor_price
        sweep_cross_direction = sweep_outputs.sweep_cross_direction
        sweep_cross_depth = sweep_outputs.sweep_cross_depth
        sweep_cross_anchor_tau_s = sweep_outputs.sweep_cross_anchor_tau_s
        sweep_cross_anchor_price = sweep_outputs.sweep_cross_anchor_price
        noise_fade_taker_mode = sweep_outputs.noise_fade_taker_mode
        noise_fade_taker_size_lots = sweep_outputs.noise_fade_taker_size_lots
        noise_fade_ma_price = sweep_outputs.noise_fade_ma_price
        noise_fade_bid_px = sweep_outputs.noise_fade_bid_px
        noise_fade_ask_px = sweep_outputs.noise_fade_ask_px
        sweep_deviation_ticks = sweep_outputs.sweep_deviation_ticks
        sweep_short_ma = sweep_outputs.sweep_short_ma_price
        sweep_long_ma = sweep_outputs.sweep_long_ma_price
        sweep_detected = sweep_outputs.sweep_detected
        sweep_reversion_bid_levels = sweep_outputs.sweep_reversion_bid_levels
        sweep_reversion_ask_levels = sweep_outputs.sweep_reversion_ask_levels
        sweep_reversion_mode = sweep_outputs.sweep_reversion_mode
        sweep_reversion_size_lots = sweep_outputs.sweep_reversion_size_lots
        sweep_exit_mode = sweep_outputs.sweep_exit_mode
        sweep_exit_lots = sweep_outputs.sweep_exit_lots
        sweep_exit_side_sign = sweep_outputs.sweep_exit_side_sign
    ...
    def _queue_pending_markouts(
        ...
        """Delegates to MarkoutTracker. Kept here to satisfy the
        AdaptiveParameterObserverContext protocol used by param_observer.py.
        """
        MarkoutTracker.queue_pending(
            symbol,
            adaptive_state,
            order_ledger=self.order_ledger,
            history_config=self.history_config,
    ...
    def _consume_ready_markouts(
        ...
        *,
        ...
        AdaptiveParameterObserverContext protocol used by param_observer.py.
        """
        return MarkoutTracker.consume_ready(
            adaptive_state,
    ...
    def _ewma(
        ...
        *,
        ...
        """Delegates to the shared implementation. Kept as a method here to
        satisfy the AdaptiveParameterObserverContext protocol used by
        param_observer.py.
        """
        return _ewma(previous, current, alpha, initialized=initialized)
